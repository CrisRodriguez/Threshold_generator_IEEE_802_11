...
public:
	...
	void set_msg(pmt::pmt_t msg) { 
		...
		dout << "Nueva lectura : " << d_msg << " ninput: " << std::endl;
			...
			} else if (pmt::is_f32vector(d_msg)){	
				d_threshold =(double) pmt::f32vector_ref(d_msg,4);
			} else {
			 // We really expected an integer or a double here, so we don't know what to do
			    throw std::runtime_error("expected a real in double format!");
			}
	...
	//Constructor!!!
sync_short_impl(unsigned int min_plateau, bool log, bool debug) :
		block("sync_short",
		...
		d_threshold(0.55),
		reject(0) { //I set a default value for threshold.
...
sync_short::sptr
sync_short::make(unsigned int min_plateau, bool log, bool debug) {
	return gnuradio::get_initial_sptr(new sync_short_impl(min_plateau, log, debug));
}
